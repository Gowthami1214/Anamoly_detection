...
def predict():
    ...
    try:
        ...
        if severity_class == 0:
            ...
            explanation = "Borderline anomaly detected. Minor deviations observed."
        elif severity_class == 2:
            explanation = "Highly abnormal anomaly detected. Significant deviations."
        elif severity_class == 3:
            explanation = "Extreme anomaly detected. Severe physiological abnormalities."
        else:
            explanation = f"{severity_label} detected."
            
        # Add pattern info only for significant anomalies
        if severity_class >= 2 and pattern_similarity != "None":
            explanation += f" The vital signs pattern resembles: {pattern_similarity}."
            
        # Add top contributors for any anomaly
        if severity_class > 0 and top_features:
            top_names = [f['feature'] for f in top_features[:3]]
            explanation += f" Primary contributors: {', '.join(top_names)}."
